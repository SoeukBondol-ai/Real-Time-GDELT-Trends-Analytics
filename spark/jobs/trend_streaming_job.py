...
def _load_wordlists() -> dict:
    """Load wordlists from JSON, falling back to a WORDLIST_PATH env var."""
    env_path = os.environ.get("WORDLIST_PATH")
    if env_path:
        path = Path(env_path)
    else:
        # Default: spark/wordlists/sentiment_words.json (relative to this script)
        path = Path(__file__).resolve().parent.parent / "wordlists" / "sentiment_words.json"
...
_wordlists = _load_wordlists()
STOP_WORDS = _wordlists["stop_words"]
POSITIVE = _wordlists["positive"]
NEGATIVE = _wordlists["negative"]
